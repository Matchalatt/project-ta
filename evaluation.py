# File: evaluation.py (Versi Final)
import pandas as pd
import numpy as np
import pymysql
...
# =====================================================================
# 1. IMPOR FUNGSI REKOMENDASI DARI SETIAP MODEL
# =====================================================================
try:
    ...
    print("Pastikan semua file recommend_*.py berada di direktori yang sama.")
    exit()
...
def get_transaction_df():
    ...
    try:
        ...
        if len(df) == 0:
            print("Peringatan: Tidak ada data transaksi yang ditemukan di database.")
            return pd.DataFrame()
        print(f"Berhasil memuat {len(df)} baris data transaksi.")
        return df
    except Exception as e:
        print(f"Koneksi database gagal: {e}")
        print("Pastikan detail koneksi di DB_CONFIG sudah benar dan database berjalan.")
        return pd.DataFrame()
    finally:
...
def main():
    ...
    for user_id, relevant_items in tqdm(test_ground_truth.items()):
        ...
        # --- [PERUBAHAN 1] Evaluasi Model Collaborative Filtering dengan train_df ---
        ...
        # --- [PERUBAHAN 2] Evaluasi Model Content-Based dengan verbose=False ---
    ...
    print("\n\n--- HASIL EVALUASI KUANTITATIF (METRIK OFFLINE) ---")
    ...
    print(f" - MAP: Metrik paling komprehensif, mengukur relevansi dan urutan. Skor tertinggi lebih baik.")
